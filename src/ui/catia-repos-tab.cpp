...
CatiaReposTab::CatiaReposTab(QWidget *parent)
    : TabView(parent)
{
...
void CatiaReposTab::initCatiaReposTab()
{
    createRepoTree();
    createLoadingView();
...
void CatiaReposTab::refreshRepos(const std::vector<ServerRepo>& repos)
{
...
void CatiaReposTab::refreshReposFailed(const ApiError& error)
{
...
std::vector<QAction*> CatiaReposTab::getToolBarActions()
{
    return repos_tree_->getToolBarActions();
}

void CatiaReposTab::showLoadingView()
{
    filter_text_->setVisible(false);
    mStack->setCurrentIndex(INDEX_LOADING_VIEW);
}

void CatiaReposTab::getPdm()
{
...
void CatiaReposTab::startRefresh()
{
    RepoService::instance()->start();
}

void CatiaReposTab::stopRefresh()
{
    RepoService::instance()->stop();
}

void CatiaReposTab::onFilterTextChanged(const QString& text)
{
