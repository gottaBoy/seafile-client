...
#include "catia-search-bar.h"

namespace {

const int kMarginRightCatiaSearchBar = 16;
//const int kMarginBottom = 5;
const int kHMargin = 10;

} // namespace

CatiaSearchBar::CatiaSearchBar(QWidget *parent)
    : QLineEdit(parent)
{
    setObjectName("mCatiaSearchBar");
    ...
    const QString style = QString(" QLineEdit#mCatiaSearchBar { "
                                      " padding-right: %1px; "
                                      " padding-left: %2px; } " )
...
void CatiaSearchBar::paintEvent(QPaintEvent* event)
{
    QLineEdit::paintEvent(event);
}

void CatiaSearchBar::resizeEvent(QResizeEvent* event)
{
    clear_button_->move(rect().right() - kMarginRightCatiaSearchBar
...
void CatiaSearchBar::onTextChanged(const QString& text)
{
    clear_button_->setVisible(!text.isEmpty());
    placeholder_label_->setVisible(text.isEmpty());
}

void CatiaSearchBar::setPlaceholderText(const QString& text)
{
